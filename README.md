...
│
├── data/
│    ├── locations.json       # 1 hub + 12 titik pelanggan area Bandung (Kiaracondong, Dago, Antapani, Cicadas, Buah Batu, Margahayu, Coblong, Sukajadi, Setiabudhi, Ujungberung, Gedebage, Cimahi, Cinambo)
...
2. **Performa O(n²)**: Dengan 10 pelanggan, algoritma ini selesai dalam hitungan mikrodetik, menjadikan biaya server hampir nol.
...
P_breakeven = 1.497 / 0.0867 ≈ Rp 17.26/liter  → dibulatkan ~Rp 17/liter
```

**Interpretasi hasil nyata:**
- Break-Even Point: **Rp 17/liter**
...
> **Catatan Penting**: Dalam kasus dataset ini, meskipun Rp 5.000 di atas break-even teoritis (Rp 17), selisih TCO aktual bergantung pada jarak. Di implementasi nyata dengan beban kurir yang lebih berat dan rute lebih panjang, break-even point akan berubah. Jalankan `python src/main.py --scenario all` untuk mendapatkan break-even dari data terkini.
...
|---------|-------------|--------|
| BBM < Rp 17/liter (di bawah break-even) | **Greedy** | Penghematan komputasi > penghematan rute |
| BBM > Rp 17/liter (di atas break-even) | **Eksak** | Penghematan rute > biaya komputasi tambahan |

**Kesimpulan**: Dengan dataset area Bandung ini, Break-Even Point berada di **Rp 17/liter**. Karena kedua skenario simulasi (Rp 5.000 dan Rp 20.000) berada jauh di atas titik ini, Algoritma Eksak lebih menguntungkan untuk dijalankan. Manajemen dapat menggunakan angka ini sebagai threshold keputusan algoritmik yang objektif.
